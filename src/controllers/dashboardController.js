const db = require('../config/database');
const { successResponse, errorResponse } = require('../utils/response');

// Get dashboard overview
exports.getDashboard = async (req, res) => {
  try {
    // Total sites
    const [sitesCount] = await db.query(
      'SELECT COUNT(*) as total FROM sites WHERE is_active = TRUE'
    );

    // Total staff
    const [staffCount] = await db.query(
      'SELECT COUNT(*) as total FROM staff WHERE is_active = TRUE'
    );

    // Total inventory entries (last 30 days)
    const [inventoryCount] = await db.query(
      'SELECT COUNT(*) as total FROM inventory WHERE date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)'
    );

    // Total DPR entries (last 7 days)
    const [dprCount] = await db.query(
      'SELECT COUNT(*) as total FROM dpr WHERE date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)'
    );

    // Today's attendance
    const [todayAttendance] = await db.query(
      'SELECT COUNT(*) as total FROM attendance WHERE date = CURDATE()'
    );

    // Recent DPR entries (last 10)
    const [recentDPR] = await db.query(
      `SELECT d.*, s.site_name, w.work_name, w.unit 
       FROM dpr d 
       INNER JOIN sites s ON d.site_id = s.id 
       INNER JOIN work_types w ON d.work_type_id = w.id 
       ORDER BY d.created_at DESC 
       LIMIT 10`
    );

    // Recent inventory updates (last 10)
    const [recentInventory] = await db.query(
      `SELECT i.*, s.site_name 
       FROM inventory i 
       INNER JOIN sites s ON i.site_id = s.id 
       ORDER BY i.created_at DESC 
       LIMIT 10`
    );

    // Active sites list
    const [activeSites] = await db.query(
      'SELECT id, site_name, site_address, start_date FROM sites WHERE is_active = TRUE ORDER BY created_at DESC'
    );

    const dashboardData = {
      stats: {
        total_sites: sitesCount[0].total,
        total_staff: staffCount[0].total,
        recent_inventory_entries: inventoryCount[0].total,
        recent_dpr_entries: dprCount[0].total,
        today_attendance: todayAttendance[0].total
      },
      recent_dpr: recentDPR,
      recent_inventory: recentInventory,
      active_sites: activeSites
    };

    successResponse(res, dashboardData, 'Dashboard data retrieved successfully');
  } catch (error) {
    console.error('Get dashboard error:', error);
    errorResponse(res, 'Failed to retrieve dashboard data', 500);
  }
};

// Get site-specific dashboard
exports.getSiteDashboard = async (req, res) => {
  try {
    const { site_id } = req.params;

    // Site details
    const [site] = await db.query(
      'SELECT * FROM sites WHERE id = ? AND is_active = TRUE',
      [site_id]
    );

    if (site.length === 0) {
      return errorResponse(res, 'Site not found', 404);
    }

    // Total staff
    const [staffCount] = await db.query(
      'SELECT COUNT(*) as total FROM staff WHERE site_id = ? AND is_active = TRUE',
      [site_id]
    );

    // Total inventory entries
    const [inventoryCount] = await db.query(
      'SELECT COUNT(*) as total FROM inventory WHERE site_id = ?',
      [site_id]
    );

    // Total DPR entries
    const [dprCount] = await db.query(
      'SELECT COUNT(*) as total FROM dpr WHERE site_id = ?',
      [site_id]
    );

    // Today's attendance
    const [todayAttendance] = await db.query(
      `SELECT COUNT(*) as present 
       FROM attendance 
       WHERE site_id = ? AND date = CURDATE() AND status = 'Present'`,
      [site_id]
    );

    // Recent DPR (last 5)
    const [recentDPR] = await db.query(
      `SELECT d.*, w.work_name, w.unit 
       FROM dpr d 
       INNER JOIN work_types w ON d.work_type_id = w.id 
       WHERE d.site_id = ? 
       ORDER BY d.date DESC, d.created_at DESC 
       LIMIT 5`,
      [site_id]
    );

    // Recent inventory (last 5)
    const [recentInventory] = await db.query(
      `SELECT * FROM inventory 
       WHERE site_id = ? 
       ORDER BY date DESC, created_at DESC 
       LIMIT 5`,
      [site_id]
    );

    // Staff list
    const [staffList] = await db.query(
      'SELECT id, staff_name, role, phone FROM staff WHERE site_id = ? AND is_active = TRUE',
      [site_id]
    );

    const siteDashboard = {
      site: site[0],
      stats: {
        total_staff: staffCount[0].total,
        total_inventory_entries: inventoryCount[0].total,
        total_dpr_entries: dprCount[0].total,
        today_present: todayAttendance[0].present
      },
      recent_dpr: recentDPR,
      recent_inventory: recentInventory,
      staff: staffList
    };

    successResponse(res, siteDashboard, 'Site dashboard retrieved successfully');
  } catch (error) {
    console.error('Get site dashboard error:', error);
    errorResponse(res, 'Failed to retrieve site dashboard', 500);
  }
};
